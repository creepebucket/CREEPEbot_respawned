...
async def distribute_event(context: Context):
    registries = get_registry()
    for rule in registries:

        try:
            if rule.check(context):
                if isinstance(rule, QqCommand) and not rule.bypass_enable_check and not context.chat_session.is_rule_enabled(rule.id):
